...
_UPD: Добавил отдельный эксепшен для ошибки валидации формы регистрации (ru.bank_x.registration_service.services.registration.RegistrationFormNotValidException)
...
_Попробовал разбить эту часть системы на слои с применением принципа инверсии зависимостей (D из SOLID). 
Контроллер теперь отдаёт RegistrationForm фасадному сервису, который отвечает за регистрацию и зависит от сервиса
сохранения в БД и сервиса верификации. В соответсвии с DIP ничего о них не знает_
...
_Перенёс энкодер в конвертер RegistrationForm->UserJpaEntity_
---
<span>5)</span>
```java
@Autowired
...
_UPD: Убрал использование @Embeddable сущности, оставил OneToOne отношение между таблицами, как и хотел изначально_
...
_Разеделил сущности. Сервисный слой не должен знать, что мы используем JPA. Вместо этого работаем с POJO, который в слое
доступа к данным конвертируется в необходимую сущность (в данном случае в @Entity JPA)_